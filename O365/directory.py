...
class User(ApiComponent):
    _endpoints = {"photo": "/photo/$value", "photo_size": "/photos/{size}/$value"}
    ...
    def __init__(self, *, parent=None, con=None, **kwargs):
        ...
        self.hire_date = parse(hire_date).astimezone(local_tz) if hire_date else None
        ...
        self.last_password_change = (
            parse(last_password_change).astimezone(local_tz)
            if last_password_change
            else None
        )
        self.legal_age_group_classification = cloud_data.get(
            cc("legalAgeGroupClassification")
        )
        self.license_assignment_states = cloud_data.get(
            cc("licenseAssignmentStates")
        )  # read only
        ...
        sign_in_sessions_valid_from = cloud_data.get(
            cc("signInSessionsValidFromDateTime")
        )  # read only
        self.sign_in_sessions_valid_from = (
            parse(sign_in_sessions_valid_from).astimezone(local_tz)
            if sign_in_sessions_valid_from
            else None
        )
        ...
        self.on_premises_sam_account_name = cloud_data.get(
            cc("onPremisesSamAccountName")
        )
    ...
    def update_profile_photo(self, photo):
        ...
        response = self.con.patch(
            url, data=photo, headers={"Content-type": "image/jpeg"}
        )
...
class Directory(ApiComponent):
    ...
    def get_current_user(self, query=None):
        ...
        if self.main_resource != ME_RESOURCE:
            raise ValueError(
                f"Can't get the current user. The main resource must be set to '{ME_RESOURCE}'"
            )
    ...
    def get_user_direct_reports(
        self, user, limit=100, *, query=None, order_by=None, batch=None
    ):
