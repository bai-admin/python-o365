...
class Group(ApiComponent):
        ...
        if recursive:
            ...
            for member in recursive_data:
                if member["@odata.type"] == "#microsoft.graph.group":
                    recursive_members = (
                        Groups(con=self.con, protocol=self.protocol)
                        .get_group_by_id(member["id"])
                        ._get_group_members_raw()
                    )
    ...
    def _get_group_members_raw(self):
        url = self.build_url(
            self._endpoints.get("get_group_members").format(group_id=self.object_id)
        )
    ...
    def get_group_owners(self):
        ...
        """
        url = self.build_url(
            self._endpoints.get("get_group_owners").format(group_id=self.object_id)
        )
...
class Groups(ApiComponent):
    ...
    def get_group_by_mail(self, group_mail=None):
        ...
        return self.group_constructor(
            parent=self, **{self._cloud_data_key: data.get("value")[0]}
        )
    ...
    def list_groups(self):
        ...
        url = self.build_url(self._endpoints.get("list_groups"))
