...
class Contact(ApiComponent, AttachableMixin):
    ...
    def __init__(self, *, parent=None, con=None, **kwargs):
        ...
        self.__emails = Recipients(
            recipients=[
                (rcp.get(cc("name"), ""), rcp.get(cc("address"), "")) for rcp in emails
            ],
            parent=self,
    ...
    def to_api_data(self, restrict_keys=None):
        ...
        if restrict_keys:
            restrict_keys.add(
                cc("givenName")
            )  # GivenName is required by the api all the time.
    ...
    def save(self):
        ...
        if not self.object_id:
            ...
            local_tz = self.protocol.timezone
            self.__created = (
                parse(self.created).astimezone(local_tz) if self.__created else None
            )
            self.__modified = (
                parse(self.modified).astimezone(local_tz) if self.__modified else None
            )
        else:
    ...
    def get_profile_photo(self, size=None):
        ...
        if size is None:
            ...
            url = self.build_url(
                self._endpoints.get("photo_size").format(id=self.object_id, size=size)
            )
        ...
        try:
            ...
            log.debug(
                "Error while retrieving the contact profile photo. Error: {}".format(e)
            )
            return None
...
class BaseContactFolder(ApiComponent):
    ...
    def get_contacts(self, limit=100, *, query=None, order_by=None, batch=None):
        ...
        if self.root:
            ...
            url = self.build_url(
                self._endpoints.get("folder_contacts").format(id=self.folder_id)
            )
