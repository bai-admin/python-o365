...
class EventRecurrence(ApiComponent):
    def __init__(self, event, recurrence=None):
        ...
        self.__occurrences = recurrence_range.get(self._cc("numberOfOccurrences"), None)
        ...
        self.__recurrence_time_zone = recurrence_range.get(
            self._cc("recurrenceTimeZone"), get_windows_tz(self.protocol.timezone)
        )
        ...
        if recurrence_range:
            self.__start_date = (
                parse(self.__start_date).date() if self.__start_date else None
            )
    ...
    def __repr__(self):
        ...
        pattern = "Daily: every {} day{}".format(
            self.__interval, "s" if self.__interval != 1 else ""
        )
        if self.__days_of_week:
            days = " or ".join(list(self.__days_of_week))
            pattern = "Relative Monthly: {} {} every {} month{}".format(
                self.__index, days, self.__interval, "s" if self.__interval != 1 else ""
            )
            if self.__first_day_of_week:
                pattern = "Weekly: every {} week{} on {}".format(
                    self.__interval, "s" if self.__interval != 1 else "", days
                )
            elif self.__month:
                pattern = "Relative Yearly: {} {} every {} year{} on {}".format(
                    self.__index,
                    days,
                    self.__interval,
            ...
            pattern = "Absolute Monthly: on day {} every {} month{}".format(
                self.__day_of_month,
                self.__interval,
                "s" if self.__interval != 1 else "",
            )
            if self.__month:
                pattern = "Absolute Yearly: on {} {} every {} year/s".format(
                    MONTH_NAMES[self.__month - 1], self.__day_of_month, self.__interval
                )
        ...
        if self.__start_date:
            ...
            if self.__end_date:
                ...
                ends_on = "up to {} occurrence{}".format(
                    self.__occurrences, "s" if self.__occurrences != 1 else ""
                )
    ...
    def to_api_data(self):
        ...
        if self.__interval and isinstance(self.__interval, int):
            ...
            if self.__days_of_week and isinstance(
                self.__days_of_week, (list, tuple, set)
            ):
                ...
                if self.__first_day_of_week:
                    ...
                    recurrence_pattern[self._cc("firstDayOfWeek")] = (
                        self.__first_day_of_week
                    )
        ...
        if self.__start_date:
            ...
            recurrence_range[self._cc("recurrenceTimeZone")] = (
                self.__recurrence_time_zone
            )

            if self.__end_date:
                recurrence_range[self._cc("type")] = "endDate"
                recurrence_range[self._cc("endDate")] = self.__end_date.isoformat()
            elif self.__occurrences is not None and isinstance(self.__occurrences, int):
                recurrence_range[self._cc("type")] = "numbered"
                recurrence_range[self._cc("numberOfOccurrences")] = self.__occurrences
    ...
    def set_monthly(
        self, interval, *, day_of_month=None, days_of_week=None, index=None, **kwargs
    ):
...
class ResponseStatus(ApiComponent):
    ...
    def __init__(self, parent, response_status):
        ...
        self.status = (
            None if self.status == "none" else EventResponse.from_value(self.status)
        )
        if self.status:
            ...
            if self.response_time:
                try:
                    ...
                    log.debug(
                        f"Couldn't parse event response time: {self.response_time}"
                    )
                    self.response_time = None
        else:
            self.response_time = None
...
class Attendee:
    ...
    def __repr__(self):
        if self.name:
            return "{}: {} ({})".format(
                self.attendee_type.name, self.name, self.address
            )
        else:
            ...
            if isinstance(attendees, str):
                ...
                for attendee in attendees:
                    ...
                    if address:
                        ...
                        # default value
                        attendee_type = attendee.get(self._cc("type"), "required")
                        self.__attendees.append(
                            Attendee(
                                address=address,
                                name=name,
                ...
                raise ValueError(
                    "Attendees must be an address string, an "
...
class Event(ApiComponent, AttachableMixin, HandleRecipientsMixin):
    ...
    def __init__(self, *, parent=None, con=None, **kwargs):
        ...
        self.__attendees = Attendees(
            event=self,
            attendees={self._cloud_data_key: cloud_data.get(cc("attendees"), [])},
        )
        ...
        if not self.online_meeting_url and self.is_online_meeting:
            self.online_meeting_url = (
                self.online_meeting.get(cc("joinUrl"), None)
        ...
        self.__recurrence = EventRecurrence(
            event=self, recurrence=cloud_data.get(cc("recurrence"), None)
        )
        ...
        self.__response_status = ResponseStatus(
            parent=self, response_status=cloud_data.get(cc("responseStatus"), {})
        )
        ...
        self.__event_type = EventType.from_value(
            cloud_data.get(cc("type"), "singleInstance")
        )
    ...
    def __repr__(self):
        if self.start.date() == self.end.date():
            return "Subject: {} (on: {} from: {} to: {})".format(
                self.subject, self.start.date(), self.start.time(), self.end.time()
            )
        else:
            return "Subject: {} (starts: {} {} and ends: {} {})".format(
                self.subject,
                self.start.date(),
                self.start.time(),
                self.end.date(),
    ...
    def show_as(self, value):
        self.__show_as = (
            value if isinstance(value, EventShowAs) else EventShowAs.from_value(value)
        )
    ...
    def online_meeting_provider(self, value):
        self.__online_meeting_provider = (
            value
            if isinstance(value, OnlineMeetingProviderType)
    ...
    def get_occurrences(
        self, start, end, *, limit=None, query=None, order_by=None, batch=None
    ):
        """
    ...
    def save(self):
        ...
        if self.object_id:
            ...
            if self.calendar_id:
                url = self.build_url(
                    self._endpoints.get("event_calendar").format(id=self.calendar_id)
                )
            else:
...
class Calendar(ApiComponent, HandleRecipientsMixin):
    ...
    def get_events(
        ...
        *,
        ...
        if include_recurring:
            ...
            if end < start:
                raise ValueError(
                    'When using "include_recurring=True", the date asigned to the "end" datetime'
    ...
    def new_event(self, subject=None):
        ...
        return self.event_constructor(
            parent=self, subject=subject, calendar_id=self.calendar_id
        )
    ...
    def get_event(self, param):
        ...
        if isinstance(param, str):
            url = self.build_url(
                self._endpoints.get("get_event").format(id=self.calendar_id, ide=param)
            )
            params = None
            by_id = True
        else:
    ...
    def get_availability(self, schedules, start, end, interval=60):
        ...
        for schedule in data:
            ...
            schedule["availabilityView"] = [
                availability_view_codes.get(code, "unkknown") for code in a_view
            ]
