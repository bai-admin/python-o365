...
class Category(ApiComponent):
    _endpoints = {"update": "/outlook/masterCategories/{id}"}
    ...
    def __repr__(self):
        return "{} (color: {})".format(
            self.name, self.color.name if self.color else None
        )
...
class Categories(ApiComponent):
    ...
    def create_category(self, name, color="auto"):
        ...
        return self.category_constructor(
            parent=self, **{self._cloud_data_key: category}
        )
