    ...
    def download(
        ...
        try:
            if chunk_size is None:
                ...
                raise ValueError(
                    "Argument chunk_size must be either 'auto' "
            ...
            with self.con.get(url, stream=stream, params=params) as response:
                if not response:
                    log.debug(
                        "Downloading driveitem Request failed: {}".format(
                            response.reason
                        )
                    )
                    return False
            ...
            log.error(
                "Error downloading driveitem {}. Error: {}".format(self.name, str(e))
            )
            return False
...
class CopyOperation(ApiComponent):
    ...
    def _request_status(self):
        ...
        self.completion_percentage = data.get(self._cc("percentageComplete"), 0)
    ...
    def get_item(self):
        ...
        :rtype: DriveItem
        """
        return self.target.get_item(self.item_id) if self.item_id is not None else None
...
class DriveItemVersion(ApiComponent, DownloadableMixin):
    ...
    def __repr__(self):
        return "Version Id: {} | Modified on: {} | by: {}".format(
            self.name,
            self.modified,
            self.modified_by.display_name if self.modified_by else None,
        )
    ...
    def download(
        self, to_path=None, name=None, chunk_size="auto", convert_to_pdf=False
    ):
...
class DriveItemPermission(ApiComponent):
    ...
    _endpoints = {"permission": "/items/{driveitem_id}/permissions/{id}"}
    ...
    def __init__(self, *, parent=None, con=None, **kwargs):
        ...
        if invitation:
            ...
            ) or invited_by.get("application", {}).get(self._cc("displayName"), None)
        ...
        ) or granted_to.get("application", {}).get(self._cc("displayName"))
    ...
    def __repr__(self):
        return "Permission for {} of type: {}".format(
            self._parent.name, self.permission_type
        )
    ...
    def update_roles(self, roles="view"):
        ...
        url = self.build_url(
            self._endpoints.get("permission").format(
...
class DriveItem(ApiComponent):
    ...
    def __init__(self, *, parent=None, con=None, **kwargs):
        ...
        if parent and not isinstance(parent, DriveItem):
            ...
            drive_id = (
                None if parent.object_id == "root" else parent.object_id
            ) or None

            # prefix with the current known drive or the default one
            resource_prefix = (
                "/drives/{drive_id}".format(drive_id=drive_id) if drive_id else "/drive"
            )
            main_resource = "{}{}".format(
                main_resource or (protocol.default_resource if protocol else ""),
                resource_prefix,
        ...
        if remote_item is not None:
            ...
            self.remote_item = self._classifier(remote_item)(
                parent=self, **{self._cloud_data_key: remote_item}
            )
            self.parent_id = self.remote_item.parent_id
            self.drive_id = self.remote_item.drive_id
            self.set_base_url(
                "drives/{}".format(self.drive_id)
            )  # changes main_resource and _base_url
        else:
            self.drive = (
                parent
                if isinstance(parent, Drive)
                else (
                    parent.drive
                    if isinstance(parent.drive, Drive)
                    else kwargs.get("drive", None)
                )
            )
    ...
    def update(self, **kwargs):
        ...
        }  # convert keys to protocol casing
        if not data:
            return False
    ...
    def copy(self, target=None, name=None):
        ...
        if response.status_code == 202:
            ...
            return CopyOperation(
                parent=self.drive, monitor_url=location, target=target_drive
            )
        else:
            ...
            return CopyOperation(
                parent=self.drive, item_id=item_id, target=target_drive
            )
    ...
    def get_version(self, version_id):
        ...
        url = self.build_url(
            self._endpoints.get("version").format(
    ...
    def share_with_link(
        ...
        data = {"type": share_type, "scope": share_scope}
...
class File(DriveItem, DownloadableMixin):
    ...
    def __init__(self, **kwargs):
        ...
        self.hashes = cloud_data.get(self._cc("file"), {}).get(self._cc("hashes"), None)
...
class Folder(DriveItem):
    ...
    def download_contents(self, to_folder=None):
        ...
        if to_folder is None:
            try:
                ...
                log.error(
                    "Could not create folder with name: {}. Error: {}".format(
                        self.name, e
                    )
                )
        ...
        for item in self.get_items(
            query=self.new_query().select("id", "size", "folder", "name")
        ):
    ...
    def search(self, search_text, limit=None, *, query=None, order_by=None, batch=None):
        ...
        url = self.build_url(
            self._endpoints.get("search").format(
    ...
    def upload_file(
        ...
        if not upload_in_chunks and file_size <= UPLOAD_SIZE_LIMIT_SIMPLE:
            # Simple Upload
            url = self.build_url(
                self._endpoints.get("simple_upload").format(
                    id=self.object_id,
            ...
            if conflict_handling:
                file_data.setdefault("item", dict())[
                    "@microsoft.graph.conflictBehavior"
                ] = conflict_handling
            if file_created_date_time:
                file_data.setdefault("item", dict()).setdefault(
                    "fileSystemInfo", dict()
                )["createdDateTime"] = file_created_date_time
            if file_last_modified_date_time:
                file_data.setdefault("item", dict()).setdefault(
                    "fileSystemInfo", dict()
                )["lastModifiedDateTime"] = file_last_modified_date_time
            ...
            if expiration_date:
                log.info(
                    "Expiration Date for this upload url is: {}".format(expiration_date)
                )
            if upload_url is None:
                log.error(
                    "Create upload session response without "
                ...
                while True:
                    ...
                    headers = {
                        ...
                        "Content-Range": "bytes {}-{}/{}".format(
                            current_bytes, current_bytes + transfer_bytes - 1, file_size
                        ),
                    }
...
class Drive(ApiComponent):
    ...
    def _update_data(self, data):
        ...
        self.owner = (
            Contact(
                con=self.con, protocol=self.protocol, **{self._cloud_data_key: owner}
            )
            if owner
            else None
        )
    ...
    def get_items(self, limit=None, *, query=None, order_by=None, batch=None):
        ...
        return self._base_get_list(
            url, limit=limit, query=query, order_by=order_by, batch=batch
        )
    ...
    def get_shared_with_me(
        self, limit=None, allow_external=False, *, query=None, order_by=None, batch=None
    ):
        ...
        return self._base_get_list(
            url, limit=limit, query=query, order_by=order_by, batch=batch, params=params
        )
    ...
    def get_item(self, item_id):
        ...
        if self.object_id:
            # reference the current drive_id
            url = self.build_url(
                self._endpoints.get("get_item").format(
                    id=self.object_id, item_id=item_id
                )
            )
        else:
            # we don't know the drive_id so go to the default drive
    ...
    def get_item_by_path(self, item_path):
        ...
        if self.object_id:
            # reference the current drive_id
            url = self.build_url(
                self._endpoints.get("get_item_by_path").format(
                    id=self.object_id, item_path=item_path
                )
            )
        else:
            # we don't know the drive_id so go to the default drive
            url = self.build_url(
                self._endpoints.get("get_item_by_path_default").format(
                    item_path=item_path
                )
            )
    ...
    def get_special_folder(self, name):
        ...
        name = (
            name
            if isinstance(name, OneDriveWellKnowFolderNames)
        ...
        if self.object_id:
            # reference the current drive_id
            url = self.build_url(
                self._endpoints.get("get_special").format(id=self.object_id, name=name)
            )
        else:
            # we don't know the drive_id so go to the default
            url = self.build_url(
    ...
    def search(self, search_text, limit=None, *, query=None, order_by=None, batch=None):
        ...
        if self.object_id is None:
            url = self.build_url(
                self._endpoints.get("search_default").format(search_text=search_text)
            )
        else:
            url = self.build_url(
                self._endpoints.get("search").format(
