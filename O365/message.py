...
class Message(ApiComponent, AttachableMixin, HandleRecipientsMixin):
    ...
    def to_api_data(self, restrict_keys=None):
        ...
        message = {
            ...
            cc("body"): {cc("contentType"): self.body_type, cc("content"): self.body},
        ...
        if self.reply_to:
            message[cc("replyTo")] = [
                self._recipient_to_cloud(recipient) for recipient in self.reply_to
            ]
        if self.attachments:
    ...
    def send(self, save_to_sent_folder=True):
        ...
        if self.object_id and not self.__is_draft:
            return RuntimeError(
                "Not possible to send a message that is not "
    ...
    def reply(self, to_all=True):
        ...
        if to_all:
            url = self.build_url(
                self._endpoints.get("create_reply_all").format(id=self.object_id)
            )
        else:
            url = self.build_url(
        ...
        headers = {
            "Prefer": self.protocol.get_service_keyword("prefer_timezone_header")
        }
    ...
    def save_message(self):
        ...
        if self.object_id and not self.__is_draft:
            ...
            allowed_changes = {
                ...
                self._cc("flag"),
                self._cc("subject"),
            }  # allowed changes to be saved by this method
    ...
    def save_draft(self, target_folder=OutlookWellKnowFolderNames.DRAFTS):
        ...
        if self.object_id:
            ...
            data.pop(
                self._cc("attachments"), None
            )  # attachments are handled by the next method call
            ...
            if isinstance(target_folder, OutlookWellKnowFolderNames):
                ...
                target_folder = getattr(
                    target_folder, "folder_id", OutlookWellKnowFolderNames.DRAFTS.value
                )

            url = self.build_url(
                self._endpoints.get("create_draft_folder").format(id=target_folder)
            )
        ...
        if not self.object_id:
            ...
            self.__created = message.get(
                ...
                message.get(self._cc("dateTimeCreated"), None),
            )
            # fallback to office365 v1.0
            self.__modified = message.get(
                self._cc("lastModifiedDateTime"),
                message.get(self._cc("dateTimeModified"), None),
            )
