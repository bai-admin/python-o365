...
from .connection import Connection, MSGraphProtocol, MSOffice365Protocol, Protocol
...
class Account:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if auth_flow_type not in ["authorization", "public", "credentials", "password"]:
            raise ValueError(
                '"auth_flow_type" must be "authorization", "credentials", "password" or "public"'
            )
        ...
        if scopes:
            del kwargs["scopes"]
            warnings.warn(
                "Since 2.1 scopes are only needed during authentication.",
                DeprecationWarning,
            )

        if auth_flow_type == "credentials" or auth_flow_type == "password":
    ...
    def is_authenticated(self) -> bool:
        ...
        return not self.con.token_backend.token_is_expired(
            username=self.con.username, refresh_token=True
        )
    ...
    def authenticate(
        ...
        *,
        ...
        handle_consent: Callable = consent_input_token,
        **kwargs,
    ) -> bool:
        ...
        if self.con.auth_flow_type in ("credentials", "password"):
            return self.request_token(None, requested_scopes=requested_scopes, **kwargs)

        raise ValueError(
            '"auth_flow_type" must be "authorization", "public", "password" or "credentials"'
        )
    ...
    def get_authorization_url(
        ...
        return self.con.get_authorization_url(
            requested_scopes, redirect_uri=redirect_uri, **kwargs
        )
    ...
    def request_token(
        ...
        **kwargs,
        ...
        if self.con.auth_flow_type == "credentials":
            ...
            elif len(requested_scopes) > 1 or requested_scopes[
                0
            ] != self.protocol.prefix_scope(".default"):
                raise ValueError(
                    'Provided scope for auth flow type "credentials" does not match '
            ...
            raise ValueError(
                f'Auth flow type "{self.con.auth_flow_type}" does not require scopes'
            )
    ...
    def get_authenticated_usernames(self) -> list[str]:
        ...
        for account in self.con.token_backend.get_all_accounts():
            ...
            if username and not self.con.token_backend.token_is_expired(
                username=username, refresh_token=True
            ):
    ...
    def address_book(
        self, *, resource: Optional[str] = None, address_book: str = "personal"
    ):
        ...
        if address_book.lower() == "personal":
            ...
            return AddressBook(
                parent=self, main_resource=resource, name="Personal Address Book"
            )
        if address_book.lower() in ("gal", "directory"):
    ...
    def planner(self, *, resource: str = ""):
        ...
        if not isinstance(self.protocol, MSGraphProtocol):
            # TODO: Custom protocol accessing OneDrive/Sharepoint Api fails here
            raise RuntimeError("planner api only works on Microsoft Graph API")
    ...
    def teams(self, *, resource: str = ""):
        ...
        if not isinstance(self.protocol, MSGraphProtocol):
            raise RuntimeError("teams api only works on Microsoft Graph API")
    ...
    def groups(self, *, resource: str = ""):
        ...
        if not isinstance(self.protocol, MSGraphProtocol):
            raise RuntimeError("groups api only works on Microsoft Graph API")

        from .groups import Groups

        return Groups(parent=self, main_resource=resource)

    def schema(self, *, resource: str = ""):
        """Get an instance to read information from Microsoft Graph Schema"""
        if not isinstance(self.protocol, MSGraphProtocol):
            raise RuntimeError("schema api only works on Microsoft Graph API")

        from .utils.schema import Schema

        return Schema(parent=self, main_resource=resource)
