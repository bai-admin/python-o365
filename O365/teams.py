...
class ChatMessage(ApiComponent):
    ...
    def __init__(self, *, parent=None, con=None, **kwargs):
        ...
        self.last_modified_date = (
            parse(last_modified).astimezone(local_tz) if last_modified else None
        )
        self.last_edited_date = (
            parse(last_edit).astimezone(local_tz) if last_edit else None
        )
        ...
        self.from_display_name = (
            from_data.get("displayName", None) if sent_from else None
        )
        self.from_type = (
            from_data.get("{}IdentityType".format(from_key)) if sent_from else None
        )
...
class ChannelMessage(ChatMessage):
    ...
    _endpoints = {"get_replies": "/replies", "get_reply": "/replies/{message_id}"}
    ...
    def send_reply(self, content=None, content_type="text"):
        ...
        data = (
            ...
            else {"body": {"contentType": content_type, "content": content}}
        )
        url = self.build_url(self._endpoints.get("get_replies"))
...
class Chat(ApiComponent):
    ...
    def __init__(self, *, parent=None, con=None, **kwargs):
        ...
        self.last_update_date = (
            parse(last_update).astimezone(local_tz) if last_update else None
        )
    ...
    def get_member(self, membership_id):
        ...
        """
        url = self.build_url(
            self._endpoints.get("get_member").format(membership_id=membership_id)
        )
        response = self.con.get(url)
        if not response:
            return None
...
class Channel(ApiComponent):
    ...
    _endpoints = {"get_messages": "/messages", "get_message": "/messages/{message_id}"}
...
class Team(ApiComponent):
    ...
    _endpoints = {"get_channels": "/channels", "get_channel": "/channels/{channel_id}"}
    ...
    def get_channel(self, channel_id):
        ...
        """
        url = self.build_url(
            self._endpoints.get("get_channel").format(channel_id=channel_id)
        )
        response = self.con.get(url)
...
class Teams(ApiComponent):
    ...
    def get_channel(self, team_id, channel_id):
        ...
        url = self.build_url(
            self._endpoints.get("get_channel").format(
