...
class BaseAttachment(ApiComponent):
    ...
    def __init__(self, attachment=None, *, parent=None, **kwargs):
        ...
        self.size = None
        self.update_raw_response_text(kwargs.get("raw_response_text", ""))
            ...
            if isinstance(attachment, dict):
                if self._cloud_data_key in attachment:
                    ...
                    if file_path is None:
                        raise ValueError(
                            'Must provide a valid "path" or "name" for the attachment'
                        )
                    ...
                    self.name = (
                        self.attachment.name if self.on_disk else attachment.get("name")
                    )
                    self.size = (
                        self.attachment.stat().st_size if self.attachment else None
                    )
    ...
    def to_api_data(self):
        ...
        """
        data = {
            "@odata.type": self._gk("{}_attachment_type".format(self.attachment_type)),
    ...
    def attach(self, api_object, on_cloud=False):
        ...
        if api_object and getattr(api_object, "attachments", None):
            if on_cloud:
                ...
                url = api_object.build_url(
                    self._endpoints.get("attach").format(id=api_object.object_id)
                )
...
class BaseAttachments(ApiComponent):
    ...
    def _track_changes(self):
        ...
        if (
            getattr(self._parent, "_track_changes", None) is not None
    ...
    def add(self, attachments, **kwargs):
            ...
            if isinstance(attachments, (list, tuple, set)):
                ...
                attachments_temp = [
                    self._attachment_constructor(
                        {self._cloud_data_key: attachment},
                        parent=self,
                        on_cloud=True,
                        raw_response_text=json.dumps(attachment),
                        **kwargs,
                    )
                ...
                raise ValueError(
                    "Attachments must be a str or Path or a "
    ...
    def download_attachments(self):
        ...
        url = self.build_url(
            self._endpoints.get("attachments").format(id=self._parent.object_id)
        )
        ...
        self.untrack = True
        self.add(
            {self._cloud_data_key: attachments},
            raw_response_text=json.dumps(attachments),
        )
        self.untrack = False
    ...
    def _update_attachments_to_cloud(self, chunk_size=None):
        ...
        for attachment in self.__attachments:
            if attachment.on_cloud is False:
                ...
                if file_size <= UPLOAD_SIZE_LIMIT_SIMPLE:
                    url = self.build_url(
                        self._endpoints.get("attachments").format(
                    ...
                    if expiration_date:
                        log.info(
                            "Expiration Date for this upload url is: {}".format(
                                expiration_date
                            )
                        )
                    ...
                    def write_stream(read_byte_chunk):
                        ...
                        while True:
                            ...
                            response = self._parent.con.naive_request(
                    ...
                    if attachment.attachment:
                        with attachment.attachment.open(mode="rb") as file:
                            ...
                            upload_completed = write_stream(
                                read_byte_chunk=read_from_file
                            )
                    else:
        ...
        for attachment in self.__removed_attachments:
            if attachment.on_cloud and attachment.attachment_id is not None:
                # delete attachment
                url = self.build_url(
                    self._endpoints.get("attachment").format(
        ...
        log.debug(
            "Successfully updated attachments on {}".format(self._parent.object_id)
        )
